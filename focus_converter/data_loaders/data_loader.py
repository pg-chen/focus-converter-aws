...
class DataLoader:
    ...
    def load_s3_pyarrow_dataset(self) -> Iterable[pl.LazyFrame]:
        s3 = s3fs.S3FileSystem(profile="cathay-finops-internal")
        dataset = ds.dataset(self.__data_path__, filesystem=s3)
        scanner = dataset.scanner(
            batch_size=DEFAULT_BATCH_READ_SIZE,
            use_threads=True,
            fragment_readahead=FRAGMENT_READ_AHEAD,
    ...
    def data_scanner(self) -> Iterable[pl.LazyFrame]:
        ...
        if self.__data_format__ == DataFormats.CSV:
            ...
            elif self.__parquet_data_format__ == ParquetDataFormat.S3_DATASET:
                yield from self.load_s3_pyarrow_dataset()
            else:
